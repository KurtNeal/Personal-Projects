...
function ScoreScreen:update(dt)
    if input:pressed('return') or input:pressed('escape') then
        Exit:play()
        timer:after(0.15, function() gotoRoom('Console') end)
    end
...
function ScoreScreen:draw()
    ...
    love.graphics.print('PRESS R TO QUICK RESTART', gw/2, gh/2 + 40, 0, 1, 1, math.floor(self.font:getWidth('PRESS R TO QUICK RESTART')/2), 0)
end